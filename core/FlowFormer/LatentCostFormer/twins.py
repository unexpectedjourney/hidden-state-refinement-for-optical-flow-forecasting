...
class GroupAttnRPEContext(nn.Module):
    ...
    def forward(self, x, size: Size_, context=None):
        ...
        q = self.q(x_qk + coords_enc).reshape(B, _h, self.ws, _w,
                                              self.ws, self.num_heads, C // self.num_heads).transpose(2, 3)
        q = q.reshape(B, _h * _w, self.ws * self.ws, self.num_heads,
                      C // self.num_heads).permute(0, 1, 3, 2, 4)

        v = self.v(x)
        k = self.k(x_qk + coords_enc)
        # concate and do shifting operation together
        kv = torch.cat([k, v], dim=-1)
        kv_up = torch.cat([kv[:batch_num, self.ws:Hp, :, :],
                          kv[:batch_num, Hp-self.ws:Hp, :, :]], dim=1)
        kv_down = torch.cat([kv[batch_num:batch_num*2, :self.ws, :, :],
                            kv[batch_num:batch_num*2, :Hp-self.ws, :, :]], dim=1)
        kv_left = torch.cat([kv[batch_num*2:batch_num*3, :, self.ws:Wp, :],
                            kv[batch_num*2:batch_num*3, :, Wp-self.ws:Wp, :]], dim=2)
        kv_right = torch.cat([kv[batch_num*3:batch_num*4, :, :self.ws, :],
                             kv[batch_num*3:batch_num*4, :, :Wp-self.ws, :]], dim=2)
        kv_center = kv[batch_num*4:batch_num*5, :, :, :]
        kv_shifted = torch.cat(
            [kv_up, kv_down, kv_left, kv_right, kv_center], dim=0)
        k, v = torch.split(kv_shifted, [self.dim, self.dim], dim=-1)

        k = k.reshape(B, _h, self.ws, _w, self.ws, self.num_heads,
                      C // self.num_heads).transpose(2, 3)
        k = k.reshape(B, _h * _w, self.ws * self.ws, self.num_heads,
                      C // self.num_heads).permute(0, 1, 3, 2, 4)

        v = v.reshape(B, _h, self.ws, _w, self.ws, self.num_heads,
                      C // self.num_heads).transpose(2, 3)
        v = v.reshape(B, _h * _w, self.ws * self.ws, self.num_heads,
                      C // self.num_heads).permute(0, 1, 3, 2, 4)

        attn = (q @ k.transpose(-2, -1)) * self.scale
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)
        attn = (attn @ v).transpose(2, 3).reshape(B,
                                                  _h, _w, self.ws, self.ws, C)
...
class GroupAttnRPE(nn.Module):
    ...
    def forward(self, x, size: Size_, context=None):
        ...
        q = self.q(x + coords_enc).reshape(B, _h, self.ws, _w, self.ws,
                                           self.num_heads, C // self.num_heads).transpose(2, 3)
        q = q.reshape(B, _h * _w, self.ws * self.ws, self.num_heads,
                      C // self.num_heads).permute(0, 1, 3, 2, 4)
...
class LocallyGroupedAttnRPEContext(nn.Module):
    ...
    def forward(self, x, size: Size_, context=None):
        ...
        coords = coords.view(B, 2, -1).permute(0, 2, 1)
        coords_enc = LinearPositionEmbeddingSine(
            coords, dim=C_qk).view(B, self.ws, self.ws, C_qk)
...
class GlobalSubSampleAttnRPEContext(nn.Module):
    ...
    def __init__(self, dim, num_heads=8, attn_drop=0., proj_drop=0., sr_ratio=1, vert_c_dim=0):
        ...
        if sr_ratio > 1:
            self.sr_key = nn.Conv2d(
                dim+vert_c_dim, dim, kernel_size=sr_ratio, stride=sr_ratio)
            self.sr_value = nn.Conv2d(
    ...
    def forward(self, x, size: Size_, context=None):
        ...
        # x:            B, Hp*Wp, C
        q = self.q(x_qk + coords_enc).reshape(B, padded_N,
                                              self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        ...
        coords = coords_grid(
            B, padded_size[0] // self.sr_ratio, padded_size[1] // self.sr_ratio).to(x.device)
        coords = coords.view(B, 2, -1).permute(0, 2, 1) * self.sr_ratio
        # align the coordinate of local and global
        coords_enc = LinearPositionEmbeddingSine(coords, dim=C)
        k = self.k(x_qk + coords_enc).reshape(B, (padded_size[0] // self.sr_ratio)*(
            padded_size[1] // self.sr_ratio), self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        v = self.v(x).reshape(B, (padded_size[0] // self.sr_ratio)*(
            padded_size[1] // self.sr_ratio), self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
...
class LocallyGroupedAttnRPE(nn.Module):
    ...
    def forward(self, x, size: Size_, context=None):
        ...
        coords = coords.view(B, 2, -1).permute(0, 2, 1)
        coords_enc = LinearPositionEmbeddingSine(
            coords, dim=C).view(B, self.ws, self.ws, C)
        # coords_enc:   B, ws, ws, C
        # x:            B, _h, _w, self.ws, self.ws, C
...
class GlobalSubSampleAttnRPE(nn.Module):
    ...
    def forward(self, x, size: Size_, context=None):
        ...
        # coords_enc:   B, Hp*Wp, C
        # x:            B, Hp*Wp, C
        q = self.q(x + coords_enc).reshape(B, padded_N,
                                           self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        ...
        # align the coordinate of local and global
        coords_enc = LinearPositionEmbeddingSine(coords, dim=C)
        k = self.k(x + coords_enc).reshape(B, (padded_size[0] // self.sr_ratio)*(
            padded_size[1] // self.sr_ratio), self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        v = self.v(x).reshape(B, (padded_size[0] // self.sr_ratio)*(
            padded_size[1] // self.sr_ratio), self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
...
class CrossGlobalSubSampleAttnRPE(nn.Module):
    ...
    def forward(self, x, tgt, size: Size_):
        ...
        # coords_enc:   B, H*W, C
        # x:            B, H*W, C
        q = self.q(x + coords_enc).reshape(B, N, self.num_heads,
        ...
        if self.sr is not None:
            ...
            tgt = self.norm(tgt)
        coords = coords_grid(
            B, size[0] // self.sr_ratio, size[1] // self.sr_ratio).to(x.device)
        coords = coords.view(B, 2, -1).permute(0, 2, 1) * self.sr_ratio
        # align the coordinate of local and global
        coords_enc = LinearPositionEmbeddingSine(coords, dim=C)
        k = self.k(tgt + coords_enc).reshape(B, (size[0] // self.sr_ratio)*(
            size[1] // self.sr_ratio), self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        v = self.v(tgt).reshape(B, (size[0] // self.sr_ratio)*(
            size[1] // self.sr_ratio), self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
...
class CrossGlobalSubSampleAttn(nn.Module):
    ...
    def forward(self, x, tgt, size: Size_):
        ...
        if self.sr is not None:
            ...
            tgt = self.sr(tgt).reshape(B, C, -1).permute(0, 2, 1)
            tgt = self.norm(tgt)
        kv = self.kv(tgt).reshape(B, -1, 2, self.num_heads,
...
class CrossBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., drop=0., attn_drop=0., drop_path=0.,
        ...
        self.attn = CrossGlobalSubSampleAttnRPE(
            dim, num_heads, attn_drop, drop, sr_ratio)
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., drop=0., attn_drop=0., drop_path=0.,
        ...
        if groupattention:
            ...
            if vert_c_dim > 0:
                self.attn = GroupAttnRPEContext(
                    dim, num_heads, attn_drop, drop, ws, cfg, vert_c_dim)
            else:
                self.attn = GroupAttnRPE(
                    dim, num_heads, attn_drop, drop, ws, cfg)
        elif ws is None:
            self.attn = Attention(dim, num_heads, False, None, attn_drop, drop)
        elif ws == 1:
            if with_rpe:
                if vert_c_dim > 0:
                    self.attn = GlobalSubSampleAttnRPEContext(
                        dim, num_heads, attn_drop, drop, sr_ratio, vert_c_dim)
                else:
                    self.attn = GlobalSubSampleAttnRPE(
                        dim, num_heads, attn_drop, drop, sr_ratio)
            else:
                self.attn = GlobalSubSampleAttn(
                    dim, num_heads, attn_drop, drop, sr_ratio)
        else:
            if with_rpe:
                if vert_c_dim > 0:
                    self.attn = LocallyGroupedAttnRPEContext(
                        dim, num_heads, attn_drop, drop, ws, vert_c_dim)
                else:
                    self.attn = LocallyGroupedAttnRPE(
                        dim, num_heads, attn_drop, drop, ws)
            else:
                self.attn = LocallyGroupedAttn(
                    dim, num_heads, attn_drop, drop, ws)
        self.drop_path = DropPath(
            drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(dim)
    ...
    def __init__(
        ...
        for i in range(len(depths)):
            self.patch_embeds.append(PatchEmbed(
                img_size, patch_size, prev_chs, embed_dims[i]))
        ...
        for k in range(len(depths)):
            _block = nn.ModuleList([block_cls(
                ...
                attn_drop=attn_drop_rate, drop_path=dpr[cur +
                                                        i], norm_layer=norm_layer, sr_ratio=sr_ratios[k],
                ws=1 if wss is None or i % 2 == 1 else wss[k]) for i in range(depths[k])])
            self.blocks.append(_block)
            cur += depths[k]

        self.pos_block = nn.ModuleList(
            [PosConv(embed_dim, embed_dim) for embed_dim in embed_dims])
