...
from core.utils.utils import coords_grid, bilinear_sampler, indexing
...
def sampler_gaussian(latent, mean, std, image_size, point_num=25):
    ...
    delta = torch.stack(torch.meshgrid(dy, dx), axis=-1).to(mean.device)
    delta_3sigma = F.sigmoid(std.permute(0, 2, 3, 1).reshape(
        B*HW, 1, 1, 1)) * STD_MAX * delta * 3  # [B*H*W, point_num**0.5, point_num**0.5, 2]
    ...
    sampled_latents = bilinear_sampler(latent, coords)
...
def sampler_gaussian_zy(latent, mean, std, image_size, point_num=25, return_deltaXY=False, beta=1):
    ...
    delta = torch.stack(torch.meshgrid(dy, dx), axis=-1).to(mean.device)
    # [B*H*W, point_num**0.5, point_num**0.5, 2]
    delta_3sigma = std.permute(0, 2, 3, 1).reshape(B*HW, 1, 1, 1) * delta * 3
...
def sampler_gaussian_fix_pyramid(latent, feat_pyramid, scale_weight, mean, image_size, point_num=25):
    ...
    sampled_latents = torch.stack(sampled_latents, dim=1)
    sampled_latents = sampled_latents.permute(
        0, 3, 4, 2, 1)  # [B, H*W, point_num, dim, layer_num]
    scale_weight = F.softmax(scale_weight, dim=2)  # [B, H*W, layer_num]
    vis_out = scale_weight
    scale_weight = torch.unsqueeze(torch.unsqueeze(
        scale_weight, dim=2), dim=2)  # [B, HW, 1, 1, layer_num]

    # [B, H*W, point_num, dim]
    weighted_latent = torch.sum(sampled_latents*scale_weight, dim=-1)
...
def sampler_gaussian_fix_MH(latent, mean, image_size, point_num=25):
    ...
    delta = torch.stack(torch.meshgrid(dy, dx), axis=-
                        1).to(mean.device).repeat(HEADS, 1, 1, 1)
    ...
    sampled_latents = bilinear_sampler(
        latent, coords)  # [B, dim, H*W*HEADS, pointnum]
    sampled_latents = sampled_latents.permute(
        0, 2, 3, 1)  # [B, H*W*HEADS, pointnum, dim]
    sampled_weights = -(torch.sum(delta.pow(2), dim=-1)) / \
        point_num  # smooth term
    return sampled_latents, sampled_weights
...
def sampler_gaussian_fix_pyramid_MH(latent, feat_pyramid, scale_head_weight, mean, image_size, point_num=25):
    ...
    sampled_latents = sampled_latents.permute(0, 3, 4, 2, 1)

    scale_head_weight = scale_head_weight.reshape(B, H*W*HEADS, -1)
    # [B, H*W*HEADS, layer_num]
    scale_head_weight = F.softmax(scale_head_weight, dim=2)
    scale_head_weight = torch.unsqueeze(torch.unsqueeze(
        scale_head_weight, dim=2), dim=2)  # [B, H*W*HEADS, 1, 1, layer_num]

    # [B, H*W*HEADS, point_num, dim]
    weighted_latent = torch.sum(sampled_latents*scale_head_weight, dim=-1)
...
def sampler(feat, center, window_size):
    ...
    delta = torch.stack(torch.meshgrid(dy, dx), axis=-1).to(center.device)
...
class MultiHeadAttention(nn.Module):
        ...
        if use_rpe:
            if rpe_bias is None:
                if self.rpe == 'element-wise':
                    ...
                    self.rpe_bias = nn.Parameter(torch.zeros(
                        heads, self.num_kv_tokens, dim // heads))
                    self.rpe_value = nn.Parameter(
                        torch.randn(self.num_kv_tokens, dim))
                else:
    ...
    def attend_with_rpe(self, Q, K, rpe_bias):
        ...
        dots = einsum('bhid, bhjd -> bhij', Q, K) * \
            self.scale  # (b hw) heads 1 pointnum
        if self.use_rpe:
            if self.rpe == 'element-wise':
                # (b hw) heads 1 pointnum
                rpe_bias_weight = einsum(
                    'bhid, hjd -> bhij', Q, rpe_bias) * self.scale
                dots = dots + rpe_bias_weight
            elif self.rpe == 'implicit':
                pass
                # (b hw) heads 1 pointnum
                rpe_bias_weight = einsum(
                    'bhid, bhjd -> bhij', Q, rpe_bias) * self.scale
                dots = dots + rpe_bias_weight
