...
class LinearPositionEncoding(nn.Module):
    ...
    def __init__(self, d_model, max_shape=(256, 256)):
        ...
        pe = torch.zeros((d_model, *max_shape))
        y_position = (torch.ones(max_shape).cumsum(
            0).float().unsqueeze(0) - 1) / max_shape[0]
        x_position = (torch.ones(max_shape).cumsum(
            1).float().unsqueeze(0) - 1) / max_shape[1]
...
class LearnedPositionEncoding(nn.Module):
    ...
    def __init__(self, d_model, max_shape=(80, 80)):
        ...
        self.pe = nn.Parameter(torch.randn(
            1, max_shape[0], max_shape[1], d_model))
